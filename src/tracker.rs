...
// NOTE: the info hash field is _not_included
...
pub struct TrackerResponse {
    ...
    ///
    /// Each peer is represented using 6 bytes. The first 4 bytes are
    /// the peer's IP address and the last 2 bytes are the peer's port number.
    pub peers: Peers,
}
...
mod peers {
    ...
    impl<'de> Visitor<'de> for PeersVisitor {
        ...
        fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
            formatter.write_str(
                "6 bytes, first 4 bytes are the peer's IP address 
