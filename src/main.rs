...
use codecrafters_bittorrent::torrent::{Keys, Torrent};
...
fn decode_bencoded_value(encoded_value: &str) -> (serde_json::Value, &str) {
    match encoded_value.chars().next() {
        ...
        Some('d') => {
            ...
            while !rest.is_empty() && !rest.starts_with('e') {
                ...
                let k = match k {
                    ...
                    k => {
                        panic!("dict key must be strings, not {k:?}")
                    }
                };
        ...
        Some('0'..='9') => {
            let (len, rest) = encoded_value
                .split_once(':')
                .expect("strign must contain a `:` after the length");
            let len: usize = len.parse().expect("length must be a number");
            let (string, remainder) = rest.split_at(len);
            return (string.to_string().into(), remainder);
        }
        _ => {}
    }
